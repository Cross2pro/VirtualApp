...
public class Case9_ThreadMonitor implements Case {
    ...
    public void hook() {
        try {
            ...
            VposedBridge.hookAllConstructors(Thread.class, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    super.afterHookedMethod(param);
                    ...
                    if (clazz != Thread.class) {
                        ...
                        VposedBridge.findAndHookMethod(clazz, "run", new ThreadMethodHook());
                    }
                    Log.d(TAG, "Thread: " + thread.getName() + " class:" + thread.getClass() +  " is created.");
                }
            });
            VposedBridge.findAndHookMethod(Thread.class, "run", new ThreadMethodHook());
